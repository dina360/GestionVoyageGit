...
<h1 class="mb-4">Gestion des Offres</h1>
		...
		<td class="im"><img src="<%= offer.getImage() %>" style="max-width: 200px; max-height: 200px;"></td>
                    ...
                    <td style="width:20px;"><img src="<%= offer.getRestaurant() %>" style="max-width: 200px; max-height: 200px;"></td>
                    <td><img src="<%= offer.getVille() %>"style="max-width: 200px; max-height: 200px;"></td>
                    <td><%= offer.getP_i()%></td>
                    <td><%= offer.getP_a() %></td>
                    <td><%= offer.getRemise() %></td>
                    <td><%= offer.getNbr() %></td>
                    <td>
		    <a href="#" onclick="modifierOffre('<%= offer.getId() %>', '<%= offer.getTitre() %>', '<%= offer.getDescription() %>', '<%= offer.getImage() %>', '<%= offer.getHotel() %>', '<%= offer.getRestaurant() %>', '<%= offer.getVille() %>', '<%= offer.getP_i() %>' ,'<%= offer.getP_a() %>', '<%= offer.getRemise() %>', '<%= offer.getNbr() %>')">
		        <i class="material-icons" data-toggle="tooltip" title="Modifier" style="font-size:25px;">&#xE254;</i>
		    </a>
		    <a href="DeleteOfferServlet?id=<%= offer.getId() %>" onclick="return confirmDelete()">
		        <i class="material-icons" data-toggle="tooltip" title="Delete" style="font-size:25px;">&#xE872;</i>
		    </a>
		</td>
...
<div class="modal fade" id="modalEditVoy" tabindex="-1" role="dialog" aria-labelledby="modalEditVoyLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formEditVoy" action="ModifierVoyageServlet" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <label for="editPrix">Prix initial</label>
                            <input type="number" class="form-control" id="editPrixI" name="prixInitial" required>
                             <label for="editPrix">Prix actuel</label>
                            <input type="number" class="form-control" id="editPrix" name="prixActuel" required>
                             <label for="editPrix">Remise</label>
                            <input type="number" class="form-control" id="editRemise" name="remise" required>
                             <label for="editPrix"> Nombre de paquets restants</label>
                            <input type="number" class="form-control" id="editNbr" name="nbr" required>
                           
                        </div>
                        <br>
                        <!-- Deuxième colonne : Images actuelles et nouvelles -->
                        <div class="form-group col-md-4 par1">
...
function fermerPopup(idPopup) {
    $('#' + idPopup).modal('hide'); // Fermer le popup en utilisant jQuery
        ...
        function handleSelectionChange(selectElement) {
            if (selectElement.value === 'voyages') {
                window.location.href = 'pakvoyAdmin.jsp';
            }
        }
        ...
        function modifierOffre(id, titre, description, image, hotel, restaurant, ville, prixI,prix,remise,nbr) {
            $('#editVoyId').val(id);
            $('#editTitre').val(titre);
