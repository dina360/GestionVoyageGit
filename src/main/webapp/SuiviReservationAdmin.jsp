...
<head>
    ...
    <title>Suiver Les reservation </title>
        ...
        <!-- -------- Non Dropdown List Item ------- -->
            ...
            <li class="custom-dropdown">
                ...
                <div class="custom-submenu">
                    ...
                    <a href="Voyage.jsp" class="custom-link">Voyages</a>
...
<div class="main-content" >
    ...
    <div class="container mt-5" >
        <h2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Liste des Réservations des Clients</h2>
<table id="gestionvoyage" class="table table-striped table-bordered table-hover fixed-width-table custom-table">
            ...
            <tbody>
                <% 
                    // Code pour récupérer et afficher les réservations
				...
				<tr>
                                <td><%= reserv.getId() %></td>
                                <td><%= reserv.getTitre() %></td>
                                <td><%= reserv.getVille_depart()%></td>
                                <td><%= reserv.getDate_depart() %></td>
                                <td><%= reserv.getDuree() %></td>
                                <td><%= reserv.getNbr_per() %></td>
                                <td><%= reserv.getMoy() %></td>
                                <td><%= reserv.getPrix_total() %></td>
                                <td><%= reserv.getClientId() %></td>
                              <td>
								     <div class="d-flex">
								        <!-- Bouton Supprimer -->
								        <div class="mr-2">
								            <a href="deleteReservation.jsp?reservId=<%= reserv.getId() %>" onclick="return confirmDelete()" class="btn btn-sm" data-toggle="tooltip" title="Supprimer">
								                <i class="material-icons text-success icon-sm" >&#xE872;</i>
								            </a>
								        </div>
								        <!-- Bouton Valider -->
								        <% if ("Valide".equals(reserv.getEtat())) { %>
								            <button class="btn btn-outline-success btn-sm" disabled>A_Validé</button>
								        <% } else { %>
								            <form method="post" action="validerReservation.jsp" class="d-inline">
								                <input type="hidden" name="reservationId" value="<%= reserv.getId() %>">
...
function confirmDelete() {
    ...
    var confirmation = confirm("Êtes-vous sûr de vouloir supprimer cette reservation ?");
